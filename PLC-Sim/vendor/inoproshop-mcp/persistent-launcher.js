...
const sessionDir = process.env.PLCSIM_INO_SESSION_DIR ||
  (runningAsLauncher ? fs.mkdtempSync(path.join(os.tmpdir(), "plcsim-ino-")) : "");
...
function ensureHost(executable, requestedArgs, options) {
  ...
  const env = {
    ...
    PLCSIM_INO_SESSION_DIR: sessionDir,
  };
