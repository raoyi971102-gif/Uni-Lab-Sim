...
当前实现（Current Implementation）按九个真实设备建立公共接口用例。L1 必须通过
真实跨进程 OPC UA/HTTP 接缝执行整张矩阵，不以调用 Handshake Agent 内部函数代替：
...
| `DEV-PLC-001` | Robot、S04 六工位、S06-S09 的可接单状态 | 只读，不改 PLC |
| `DEV-S1-001` | S1 清洗与补液状态 HTTP 响应 | 只读；当前只在 L1 必跑 |
| `DEV-S04-001` | 六个磁搅位逐位执行搅拌工艺 | 六套请求、完成与准入全部复位 |
| `DEV-S05-001` | 拍照位在位、完成与 OK 结果 | 只读；无拍照请求节点，证据为 partial |
| `DEV-S06-001` | 一号溶液参数提交、完成和准入撤销 | 参数、请求、完成与准入复位 |
| `DEV-S07-001` | 粉罐扫码工艺完成码 | 请求、完成与准入复位 |
| `DEV-S08-001` | 开盖、关盖与瓶盖暂存位见证 | 瓶盖见证和握手恢复初态 |
| `DEV-S09-001` | L1-L4 安全位工艺完成码 | 请求、完成与准入复位 |
| `DEV-S09-002` | L1/L2 测密度完成码与首组抽/放液天平结果 | 请求、完成与准入复位 |
...
新增步骤动作应保持小接口。目前 DSL 有 `write`、`assert`、`assert_greater`、`wait`、
`sleep` 和只允许访问环境已声明服务根地址的 `http`；复杂故障
注入应通过供应商提供的受控环境适配器实现，不能在公共运行器里硬编码实验室设备常量。

## 7. 已知缺口与下一步

当前运行器已经具备 L3/L4 真机连接、动作执行和现场证据归档路径，并已补齐九设备用例。
其中 S1 因不属于 PLC 候选包且外部认证/地址未冻结，仅在 L1 必跑；S05 因当前点表没有
独立拍照请求节点，只能验证既有完成与判定结果。除此之外，当前点表仍缺少显式
